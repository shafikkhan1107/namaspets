	...
	<div class="container">
		<div class="row footer-row">
			<div class="col-md-3 item">
				<div>
					<a href="{{url('/')}}"><img class="img-fluid footer-logo" data-bss-hover-animate="swing" src="{{ uploaded_asset(get_setting('footer_logo')) }}" /></a>
				</div>
			</div>
			<div class="col-md-3 item">
				<h3 class="head-3"><strong> IMPORTANT LINKS </strong></h3>
				<ul>
					<li>
						<a href="{{url('search')}}">
			...
			?>
			<div class="col-md-3 item">
				<h3 class="head-3"><strong>Shop by Categories</strong></h3>
				<ul>
					@foreach ($categories as $key => $category)
					<li>
						<a href="{{ route('products.category', $category->slug) }}"><strong>{{  $category->getTranslation('name') }}</strong></a>
					</li>
					@endforeach
				</ul>
			</div>
			...
			<div class="col-md-3 item text">
				<h3 class="head-3"><strong>Newsletter Subscription</strong></h3>
				<form class="form-inline" method="POST" action="{{ route('subscribers.store') }}">
					@csrf 
					<input type="email" class="email-box" placeholder="Your Email Address" name="email" required="">
					
					<button class="btn btn-primary subsc-btn" type="submit">
						Subscribe Now
					</button>
				</form>
			</div>
